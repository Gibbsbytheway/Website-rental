...
Le code de ce dépôt correspond au thème `wp-content/themes/asteria-pulsar`, installé sur une instance WordPress standard hébergée chez Hostinger. WordPress core, la base de données et les médias uploadés ne sont volontairement pas versionnés ici (voir `.gitignore`) : seul le code du thème l'est.
...
2. **Thème** : copier le dossier `wp-content/themes/asteria-pulsar` dans `wp-content/themes/` de l'installation, puis l'activer dans **Apparence > Thèmes**. Idéalement via le déploiement Git automatique Hostinger (hPanel > Site web > Avancé > Git) pointé sur ce dépôt/branche, avec le répertoire de déploiement réglé sur `wp-content/themes/asteria-pulsar`.
